...
def test_get_readings_ppe_prefix_and_meter_id_fallback(storage: CanonicalStorage):
    ...
    # Querying with '900001' finds both
    res = storage.get_readings(ppe_id="900001", resolution="1h")
    assert len(res) == 2

    # Querying with 'PPE_900001' finds both
    res_pref = storage.get_readings(ppe_id="PPE_900001", resolution="1h")
    assert len(res_pref) == 2

    # Querying with meter_id finds both
    res_meter = storage.get_readings(ppe_id="other_ppe", meter_id="10000001", resolution="1h")
...
def test_invoice_reconciliation_storage_and_approval(storage: CanonicalStorage):
    ...
    storage.save_invoice_reconciliation(recon, ppe_id="590000000000000001")

    saved = storage.get_invoice_reconciliation("FES/00017")
    assert saved is not None
    assert saved.invoice_number == "FES/00017"
    assert saved.status == "MATCH"
    assert len(saved.line_variances) == 2
    assert saved.variance_gross == Decimal("0.00")

    # Test user approval workflow
    ok = storage.set_reconciliation_approval("FES/00017", approved=True, approved_by="admin")
    assert ok is True
